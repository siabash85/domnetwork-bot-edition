        ...
        <base-skeleton animated :loading="loading">
            ...
            <template #default>
                <div class="flex justify-between mb-12 items-center">
                    <h2 class="text-xl">لیست پلتفرم ها</h2>
                    <v-btn
                        :to="{ name: 'panel-platforms-create' }"
                        color="blue-accent-2"
                    >
                        ایجاد پلتفرم
                    </v-btn>
                ...
                <v-table fixed-header height="700px">
                    ...
                    <tbody>
                        <tr v-for="item in platforms" :key="item.name">
                            <td>
...
const handleDelete = async () => {
    const { data } = await ApiService.delete(
        `/api/panel/guide/platforms/${selected_item.value.id}`
    );
    if (data.status == 200) {
